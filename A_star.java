...
public class A_star {
	...
	public double CalcH(Node a, Node b){
		double H;
		H =Math.max(Math.abs(a.get_X() - b.get_X()), Math.abs(a.get_Y() - b.get_Y()));   
		return H;
	}
	...
	public ArrayList<Node> GetSurroundNodes(Node node,Graph g){
        ...
        lu = g.get_node((node.get_X() - 1), (node.get_Y() - 1));
        ru = g.get_node((node.get_X() - 1), (node.get_Y() + 1));
        ld = g.get_node((node.get_X() + 1), (node.get_Y() - 1));
        rd = g.get_node((node.get_X() + 1), (node.get_Y() + 1));
        ...
        if(lu.get_reachable() == true) {
        	list.add(lu);
        }
        if(ru.get_reachable() == true) {
        	list.add(ru);
        }
        if(ld.get_reachable() == true) {
        	list.add(ld);
        }
        if(rd.get_reachable() == true) {
        	list.add(rd);
        }
